...
import gixy.core.severity as severity
...
class TestSecurityProtectiveValue:
    ...
    def test_cache_control_must_revalidate_is_protective(self):
        ...
        assert is_security_protective_value('cache-control', ['must-revalidate']) is True
    ...
    def test_cache_control_combined_directive_is_protective(self):
        ...
        assert is_security_protective_value('cache-control', ['private, no-cache, no-store']) is True
    ...
    def test_cache_control_public_max_age_is_not_protective(self):
        ...
        assert is_security_protective_value('cache-control', ['public, max-age=86400']) is False
    ...
    def test_expires_epoch_is_protective(self):
        ...
        assert is_security_protective_value('expires', ['Thu, 01 Jan 1970 00:00:00 GMT']) is True
    
    def test_expires_future_is_not_protective(self):
        """Expires: <future date> is just caching - NOT security."""
        assert is_security_protective_value('expires', ['Thu, 01 Jan 2030 00:00:00 GMT']) is False
    ...
    def test_content_disposition_attachment_is_protective(self):
        ...
        assert is_security_protective_value('content-disposition', ['attachment']) is True
    ...
    def test_content_disposition_attachment_with_filename_is_protective(self):
        ...
        assert is_security_protective_value('content-disposition', ['attachment; filename="file.pdf"']) is True
...
class TestPluginSeverityClassification:
    ...
    def test_always_secure_headers(self, plugin):
        ...
        for header in always_secure:
            assert header in plugin.always_secure_headers, f"{header} should be in always_secure_headers"
    ...
    def test_conditional_headers_defined(self):
        ...
        for header in expected_conditional:
            assert header in CONDITIONAL_SECURITY_HEADERS, f"{header} should be in CONDITIONAL_SECURITY_HEADERS"
...
class TestMegaManSecScenario:
    ...
    def test_our_improvement_public_is_low(self):
        ...
        assert is_security_protective_value('cache-control', ['public, max-age=3600']) is False
